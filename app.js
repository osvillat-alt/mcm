...
const sendCartBtn = document.getElementById("sendCartWhatsApp");
...
function buildCartWhatsApp(){
  ...
  const lines = cart.map((it, i) => {
    const qty = it.qty || 1;
    const priceTxt = (it.price !== "" && it.price != null) ? ` - ${money(it.price)}` : "";
...
function addToCart(product){
  ...
  if (existing){
    ...
    cart.push({ id: product.id, name: product.name || "Producto", price: product.price ?? product.priceFrom ?? "", qty: 1 });
  }
  saveCart();
  alert(`Agregado: ${product.name || "Producto"} ✅`);
}
...
function buildCartWhatsApp(){
  ...
  const lines = cart.map((it, idx) => {
    ...
    if (!Number.isNaN(p)) total += p * qty;

    const priceTxt = (it.price !== "" && it.price != null) ? ` - $${it.price}` : "";
    return `${idx+1}) ${it.name} x${qty}${priceTxt}`;
  }).join("\n");
...
function renderProducts(list) {
  ...
  <button class="btn primary" data-action="order" data-id="${p.id}">
    Pedir
  </button>
  ...
  return `
    <article class="card product">
      ...
      <div class="body">
        ...
        <div class="product-actions">
          <button class="btn primary" data-action="order" data-id="${p.id}">Pedir</button>
          <button class="btn secondary" data-action="add" data-id="${p.id}">Agregar</button>
        </div>
      </div>
    </article>
  `;
